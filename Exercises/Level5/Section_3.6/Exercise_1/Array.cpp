...
namespace francis{
    namespace Containers{
        ...
        Array::Array() : m_size(10), m_data(new francis::CAD::Point[10]) {
            for (int i = 0; i < m_size; ++i) {
                m_data[i] = francis::CAD::Point(); // Initialize with default Point objects
            }
        }

        Array::Array(int size) : m_size(size), m_data(new francis::CAD::Point[size]) {
            // Initialize each point to default
            for (int i = 0; i < m_size; ++i) {
                m_data[i] = francis::CAD::Point(); // Using default constructor of Point
            }
        }

        Array::Array(const Array& arr) : m_size(arr.m_size), m_data(new francis::CAD::Point[arr.m_size]) {
            for (int i = 0; i < m_size; ++i) {
        ...
        Array::~Array() {
            delete[] m_data;
            std::cout << "bye my Array.. (Destructor called)." << std::endl;
        }
        ...
        void Array::SetElement(int index, const francis::CAD::Point& p) {
            ...
            if (index < 0 || index >= m_size) {
                ...
                throw -1;  // Throw an integer as an exception
            }
            m_data[index] = p;
        }
        ...
        francis::CAD::Point& Array::operator[](int index) {
            // If the index is out of bounds, return the first element
            if (index < 0 || index >= m_size) {
                std::cerr << "Index out of bounds, returning first element." << std::endl;
                throw -1;
        ...
        const francis::CAD::Point& Array::operator[](int index) const {
            if (index < 0 || index >= m_size) {
        ...
        Array& Array::operator=(const Array& source) {
            ...
            m_data = new francis::CAD::Point[m_size];
